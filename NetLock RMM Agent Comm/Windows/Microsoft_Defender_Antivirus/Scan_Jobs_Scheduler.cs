...
namespace Windows.Microsoft_Defender_Antivirus
{
    internal class Scan_Jobs_Scheduler
    {
        ...
        {
            Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Jobs.Check_Execution", "Check scan job execution", "Start");
            ...
            try
            {
                ...
                List<Scan_Job> scan_jobItems = JsonSerializer.Deserialize<List<Scan_Job>>(Windows_Worker.policy_antivirus_scan_jobs_json);

                // Write each scan job to disk if not already exists
                foreach (var job in scan_jobItems)
                {
                    Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Jobs.Check_Execution", "Check if scan job exists on disk", "Job: " + job.name + " Job id: " + job.id);
                    ...
                    {
                        Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Jobs.Check_Execution", "Check if scan job exists on disk", "false");
                ...
                {
                    Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Jobs.Check_Execution", "Clean old scan jobs", "Job: " + file);
                    ...
                    if (!found)
                    {
                        Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Jobs.Check_Execution", "Clean old scan jobs", "Delete job: " + file);
                        File.Delete(file);
                    }
                }
                ...
                foreach (var job in Directory.GetFiles(Application_Paths.program_data_microsoft_defender_antivirus_scan_jobs))
                {
                    ...
                    if (!job_item.enabled)
                    {
                        Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Jobs.Check_Execution", "Check scan job execution", "Job disabled");
                    ...
                    if (execute)
                    {
                        Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Jobs.Check_Execution", "Execute scan job", "name: " + job_item.name + " id: " + job_item.id);

                        //Set Options for scan job
                        Helper.PowerShell.Execute_Command("Microsoft_Defender_AntiVirus.Scan_Jobs_Scheduler.Check_Execution.Set_CPU_Load", "Set-MpPreference -ScanAvgCPULoadFactor " + job_item.scan_settings_cpu_usage, 30);
                        Helper.PowerShell.Execute_Command("Microsoft_Defender_AntiVirus.Scan_Jobs_Scheduler.Check_Execution.Set_On_Battery", "Set-MpPreference -EnableFullScanOnBatteryPower $" + job_item.scan_settings_scan_on_battery, 30);

                        if (job_item.scan_settings_network_drives)
                            Helper.PowerShell.Execute_Command("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution.Scan_Mapped_Network_Drives", "Set-MpPreference -DisableScanningMappedNetworkDrivesForFullScan $false", 30);
                        else
                            Helper.PowerShell.Execute_Command("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution.Scan_Mapped_Network_Drives", "Set-MpPreference -DisableScanningMappedNetworkDrivesForFullScan $true", 30);

                        if (job_item.scan_settings_removable_disks)
                            Helper.PowerShell.Execute_Command("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution.Scan_Removable_Drives", "Set-MpPreference -DisableRemovableDriveScanning $false", 30);
                        else
                            Helper.PowerShell.Execute_Command("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution.Scan_Removable_Drives", "Set-MpPreference -DisableRemovableDriveScanning $true", 30);
                        ...
                        {
                            Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution", "Execute quickscan.", job_item.name);
                            result = Helper.PowerShell.Execute_Command("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution.Execute_Quickscan", "Start-MpScan -ScanType QuickScan", -1);
                        }
                        else if (job_item.scan_type == 1) // full scan
                        {
                            Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution", "Execute fullscan.", job_item.name);
                            result = Helper.PowerShell.Execute_Command("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution.Execute_Fullscan", "Start-MpScan -ScanType FullScan", -1);
                        }
                        else if (job_item.scan_type == 2) // custom scan
                        {
                            ...
                            {
                                Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution", "Execute customscan.", job_item.name + " directory: " + directory.directory);
                                result = Helper.PowerShell.Execute_Command("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution.Execute_Customscan", "Start-MpScan -ScanType CustomScan -ScanPath " + directory.directory, -1);
                            }
                        }

                        // Insert event
                        Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Jobs.Check_Execution", "Scan job executed", "name: " + job_item.name + " id: " + job_item.id + " result: " + result);

                        if (Global.Configuration.Agent.language == "en-US")
                            Global.Events.Logger.Insert_Event("0", "Microsoft Defender Antivirus", "Scan job completed. NetLock report.", "Scan job: " + job_item.name + " (" + job_item.description + ") " + Environment.NewLine + Environment.NewLine + "Result: " + "Cannot be retrieved. We are currently not aware of any way to directly determine the result of a scan job executed with PowerShell. Do you have an idea? Feel free to contact us.", Windows_Worker.microsoft_defender_antivirus_notifications_json, 0, 0);
                        ...
                        Logging.Microsoft_Defender_Antivirus("Microsoft_Defender_AntiVirus.Scan_Jobs.Check_Execution", "Execution finished", "name: " + job_item.name + " id: " + job_item.id);
                    }
                    else
            ...
            {
                Logging.Error("Microsoft_Defender_AntiVirus.Scan_Jobs.Check_Execution", "General Error", ex.ToString());
            }
        }
    }
}
